...
output$map_chart = renderHighchart({
  ...
  agg_i = aggregate(confirmed$X3.15.20, by=list(confirmed$Country.Region), sum)
  agg_ii = aggregate(confirmed$X3.14.20, by=list(confirmed$Country.Region), sum)
  agg_iii = aggregate(confirmed$X3.13.20, by=list(confirmed$Country.Region), sum)
...
output$line_chart = renderHighchart({
  ...
  if (derivative == 'zero') {
    dwnlm = genTotal_i
    label_title = 'Current cases (# persons)'
    dates = seq(from = as.Date('2020/01/22'), to = as.Date('2020/03/15'), by = "day")
    }
  
  if (derivative == 'first') {
    dwnlm = genTotal_ii
    label_title = 'Last rate of growth (#persons/day)'
    dates = seq(from = as.Date('2020/01/23'), to = as.Date('2020/03/15'), by = "day")
    }
  
  if (derivative == 'second') {
    dwnlm = genTotal_iii
    label_title = 'Diff. in last rates of growths ((#persons/day)/day)'
    dates = seq(from = as.Date('2020/01/24'), to = as.Date('2020/03/15'), by = "day")
  }
